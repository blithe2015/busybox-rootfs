#! /bin/sh
# To build static applications for the Qiuniu core, we need to split
...
the Necro POSIX/Qiuniu interface in two stages, so that symbols
...
Example:
$1 -v gcc -o foo foo.o -Wl,@/usr/necro/lib/qiuniu.wrappers -L/usr/necro/lib -lqiuniu -lmodechk -lpthread -lrt
will print and run:
+ gcc -o foo.tmp -Wl,-Ur -nostdlib foo.o -Wl,@/usr/necro/lib/qiuniu.wrappers -Wl,@/usr/necro/lib/modechk.wrappers -L/usr/necro/lib
+ gcc -o foo foo.tmp -L/usr/necro/lib -lqiuniu -lmodechk -lpthread -lrt
+ rm foo.tmp
EOF
}
...
while test $# -gt 0; do
	...
	*libqiuniu.so*|*libqiuniu.a|-lqiuniu)
